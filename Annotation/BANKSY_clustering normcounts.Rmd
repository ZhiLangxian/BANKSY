...
input_spe_filename <- "Annotation_lv1_normcounts.rds"  # Specify the 'spe' file name
...
if (subset_flag) {
  subset_variable <- "lv1.anno_bank"  # Specify the column in 'spe' to subset by
  cat("Choose the cell type variable from:\n")
  cat(unique(spe[[subset_variable]]), sep = "\n")
  value_to_exclude <- c("Keratinocytes","Melanocytes","Immune cells" ,"Sweat gland", "Undefined","Schwann cells","Adipocytes","Endothelial cells","Smooth muscle cells")  # Specify the value to exclude when subsetting the 'spe'
}
...
resolution <- 0.5 # Specify the resolution for clustering
...
cell_type_anno <- c("FB_level2") # Specify the cell types that are aimed to annotate
  ...
  with_progress({
    ...
    banksy_results <- future.apply::future_lapply(spe_subsets, function(spe) {
      ...
      Banksy::computeBanksy(
        spe,
        assay_name = "normcounts",##### New change
        compute_agf = TRUE,
        seed = 1000,
        k_geom = k_geom
      )
