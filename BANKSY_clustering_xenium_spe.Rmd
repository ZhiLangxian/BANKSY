...
input_spe_filename <- "annotation_level2_repeat1_FB_reso1_.rds"  # Specify the 'spe' file name
...
norm_scale_flag <- FALSE  # Set to TRUE to perform normalization and scaling or FALSE to skip it
...
if (subset_flag) {
  ...
  value_to_exclude <- c("Appendage-associated cells","Immune cells","Keratinocytes","Endothelial cells","Smooth muscle cells","Melanocytes" ,"Undefined","Adipocytes","Schwann cells","Fibroblasts"
    )  # Specify the value to exclude when subsetting the 'spe'
}
...
resolution <- 1.5 # Specify the resolution for clustering
...
cell_type_anno <- c("mix") # Specify the cell types that are aimed to annotate

# Parameters for the function find_markers_banksy_custom_fx
matrix_used <- "logcounts"  # Specify the matrix used for this function.
...
# spe2 <- SpatialExperiment(assay = list(counts =seurat@assays$RNA$counts, normcounts=as.matrix(seurat@assays$RNA$data), scalenormcounts = seurat@assays$RNA$scale.data),
#                          colData = seurat@meta.data,
...
assay(spe, 'counts') <- as(assay(spe, 'counts'), 'dgCMatrix')
assay(spe, 'logcounts') <- as(assay(spe, 'logcounts'), 'dgCMatrix')

cat("Log normalization performed\n")
...
if (subset_flag == FALSE) {
  ...
  for (SampleId in unique_sample_ids) {
    spe_subsets[[SampleId]] <- spe[, spe[[roi_variable]] == SampleId]
  }
  ...
  with_progress({
    ...
    banksy_results <- future.apply::future_lapply(spe_subsets, function(spe) {
      p()  # Update progress
     
      Banksy::computeBanksy(
        spe,
        assay_name = "logcounts",##### New change
        compute_agf = TRUE,
        seed = 1000,
        k_geom = k_geom
      )
...
set.seed(1000)
message("Start running Banksy PCA...")
spe_merged <- Banksy::runBanksyPCA(spe_merged,
                                   use_agf = TRUE,
                                   lambda = lambda,
                                   seed=1000,
                                   group ="SampleId",##### New change
                                   npcs=50)
message("Finished Banksy PCA.")
 
spe_merged <- Banksy::runBanksyUMAP(spe_merged,
                                    use_agf = TRUE,
                                    assay_name = NULL ,##### New change
                                    lambda = lambda,
                                    seed=1000)
...
```{r mark-fun}
## find marker function for log-normalized data
find_markers_banksy_custom_fx <- function(spe,matrix_used = "logcounts",
                                          ident_banksy = TRUE,
  ...
  seurat <- CreateSeuratObject(counts = assays(spe)[["counts"]], meta.data = data.frame(colData(spe)[, cnames_spe]))
  ...
  seurat@assays$RNA$data <- assays(spe)[["logcounts"]] # Fill the data slot with the log counts
  ...
  assays(spe, withDimnames = FALSE)[["logcounts_smooth"]] <- assays(spe)[["logcounts"]] %*% smoother
  ...
  seurat_bank_smooth@assays$RNA$data <- seurat_bank_smooth@assays$RNA$counts
  ...
  seurat_smooth <- CreateSeuratObject(counts = assays(spe)[["logcounts_smooth"]], meta.data = data.frame(colData(spe)[, cnames_spe]))
  ...
  seurat_smooth@assays$RNA$data <- seurat_smooth@assays$RNA$counts
  ...
  seurat_obj <- switch(
    matrix_used,
    "logcounts" = seurat,
    "logcounts_smooth" = seurat_smooth,
    "banksy" = seurat_bank,
    "banksy_smooth" = seurat_bank_smooth,
    stop(
      "Invalid matrix_used. Must be one of 'logcounts', 'banksy', 'banksy_smooth', 'logcounts_smooth'."
    )
  )
